...
export function getApiRegistryColumns(config: {
  ...
  return [
    columnHelper.accessor(
      (row) => row.definitions?.[0]?.label ?? row.name ?? '',
      {
        id: 'name',
            ...
            <Tooltip
              content={
                <Track direction="vertical" gap={4}>
                  <span><strong>{t('apiRegistry.tooltip.fullName')}:</strong> {fullName}</span>
                  <span><strong>{t('apiRegistry.tooltip.description')}:</strong> {description}</span>
                  <span><strong>{t('apiRegistry.tooltip.method')}:</strong> {method}</span>
                  <span><strong>{t('apiRegistry.tooltip.url')}:</strong> {url}</span>
                </Track>
              }
            >
    ...
    columnHelper.accessor(
      (row) => verificationMap[row.endpointId]?.lastTestAt ?? null,
      {
        id: 'lastTest',
    ...
    columnHelper.accessor(
      (row) => verificationMap[row.endpointId]?.verificationStatus ?? 'unverified',
      {
        id: 'status',
    ...
    columnHelper.accessor(
      (row) => verificationMap[row.endpointId]?.schemaCaptured ?? false,
      {
        id: 'schema',
        ...
        return (
          <Track gap={8}>
            <Button
              ...
              size="s"
              onClick={() => config.onCopy(endpoint)}
            >
            ...
            <Button
              appearance="secondary"
              size="s"
              onClick={() => config.onEdit(endpoint)}
            >
            ...
            <Button
              appearance="secondary"
              size="s"
              onClick={() => config.onDelete(endpoint)}
            >
