  ...
  return (
    <>
      <Track direction="horizontal" align="left" style={{ width: '100%', padding: 16 }} gap={16}>
        <FormInput
          name={'name'}
          label={t('serviceFlow.popup.name').toString()}
          placeholder={t('serviceFlow.popup.name') + ''}
        />
      </Track>
      {rules.map((rule, index) => (
        ...
        >
          <Track direction="horizontal" justify="between" gap={16} style={{ width: '100%' }}>
            <span>{`${t('serviceFlow.popup.rule')} ${index + 1}`}</span>
            <Button onClick={() => setRules(rules.filter((x) => x.id !== rule.id))} appearance="text">
              {' '}
              x{' '}
            </Button>
          </Track>
          <Track direction="horizontal" gap={16} style={{ width: '100%' }}>
            <FormSelect
      ...
      <Track direction="horizontal" justify="center" style={{ width: '100%', padding: 16 }} gap={16}>
        <Button onClick={() => setRules([...rules, { id: uuidv4() }])} appearance="text">
          {' '}
          {t('serviceFlow.popup.add')}{' '}
        </Button>
      </Track>
    </>
  );
