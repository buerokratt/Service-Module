      ...
      {items.map((value, index) => (
        ...
        >
          ...
          {index === items.length - 1 && (
            <button type="button" onClick={addItem} className={'control-button'}>
              <Icon icon={<LuPlus size={16} />} />
            </button>
          )}

          {index > 0 && (
            <button type="button" onClick={() => removeItem(index)} className={'control-button'}>
              <Icon icon={<LuMinus size={16} />} />
            </button>
          )}

          {index === 0 && <InfoTooltip name={tooltipText ?? ''} />}
        </div>
      ))}
    </div>
  );
